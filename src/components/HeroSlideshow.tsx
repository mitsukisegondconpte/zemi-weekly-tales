  ...
  return (
    <div className="relative overflow-hidden gradient-brand min-h-[300px] md:min-h-[400px] w-full">
      ...
      <div className="relative flex items-center h-full p-6 md:p-10">
        <button onClick={() => setCurrent((prev) => (prev - 1 + topNovels.length) % topNovels.length)}
        ...
        <div className="flex flex-col md:flex-row items-center gap-6 w-full animate-fade-in" key={novel.id}>
          <Link to={`/novel/${novel.id}`}
            className="w-32 h-44 md:w-40 md:h-56 rounded-xl overflow-hidden shrink-0 hover:scale-105 transition-transform shadow-2xl">
            {novel.coverUrl ? (
        ...
        <button onClick={() => setCurrent((prev) => (prev + 1) % topNovels.length)}
        ...
        {topNovels.map((_, i) => (
          <button key={i} onClick={() => setCurrent(i)}
            className={`rounded-full transition-all duration-300 ${
              i === current ? "w-6 h-2 bg-primary-foreground" : "w-2 h-2 bg-primary-foreground/40 hover:bg-primary-foreground/60"
            }`} />
        ))}
      </div>
    </div>
